...
function LineGraph(argsMap) {
	 ...
	 */
	this.slideData = function(newData,updatedXValue) {
		// validate data
		var tempData = processDataMap(newData);
			
		//alert("Existing startTime: " + data.startTime + "  endTime: " + data.endTime);
		//alert("New startTime: " + tempData.startTime + "  endTime: " + tempData.endTime);
		...
		{	
			data.startTime = new Date(updatedXValue + (data.step * numSteps));
			updatedXValue = null;
		}
		else
		...
		//alert(data.startTime.getTime());
		//alert("Updated startTime: " + data.startTime + "  endTime: " + data.endTime);
		...
		*/
		...
		$("#yaxis-id").remove();
		//alert(AllData.toSource());
		//alert('min:'+d3.min(AllData, function(d) { return d })+'  max:'+d3.max(AllData, function(d) { return d }));
		var yminVal = Math.round(d3.min(data.values[0], function(d) { return d }))<Math.round(d3.min(data.values[1], function(d) { return d }))?Math.round(d3.min(data.values[0], function(d) { return d })):Math.round(d3.min(data.values[1], function(d) { return d }));
		var ymaxVal = Math.round(d3.max(data.values[0], function(d) { return d }))>Math.round(d3.max(data.values[1], function(d) { return d }))?Math.round(d3.max(data.values[0], function(d) { return d })):Math.round(d3.max(data.values[1], function(d) { return d }));
		//alert(calculateMaxY(data, 'left'));
		if(yminVal == ymaxVal && ymaxVal<0)
			ymaxVal = ymaxVal-Math.round((ymaxVal/10));
		else if(yminVal == ymaxVal && ymaxVal>0)
			yminVal = yminVal-Math.round((yminVal/10));
		
		yLeft.domain([yminVal == ymaxVal ? yminVal-Math.round((yminVal/10)):yminVal,ymaxVal]);
		graph.append("svg:g")
			.attr("id", "yaxis-id")
	...
	var _init = function() {
		...
		margin[1] = getOptionalVar(argsMap, 'marginRight', 60)
		margin[2] = getOptionalVar(argsMap, 'marginBottom', 35) // marginBottom allows fitting the legend along the bottom
		margin[3] = getOptionalVar(argsMap, 'marginLeft', 117) // marginLeft allows fitting the axis labels
	...
	var initX = function() {
		...
		x = d3.scale.linear().domain([data.startTime.getTime(), data.endTime.getTime()]).range([0, w]);
		
		// create yAxis (with ticks)
		//xAxis = d3.svg.axis().scale(x).tickFormat(d3.time.format("%H:%M:%S"));
	...
	var createGraph = function() {
		...
		lineFunction = d3.svg.line()
			...
			.defined(function(d) {
				...
				// for positive and negative value
				return true;
	...
	*/
	var displayValueLabelsForPositionX = function(xPosition, withTransition) {
		...
		.text(function(d, i) {
			...
			// commented for displaying empty
			return '';
		})
		.attr("x", function(d, i) {
